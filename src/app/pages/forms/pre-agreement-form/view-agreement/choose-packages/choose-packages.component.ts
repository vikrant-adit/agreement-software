...
export class ChoosePackagesComponent implements OnInit {
    ...
    @Output() phoneSelectedChange: EventEmitter<boolean> = new EventEmitter<boolean>(); // Output to parent
    @Output() verificationSelectedChange: EventEmitter<boolean> = new EventEmitter<boolean>(); // Output to parent
    @Output() analyticsSelectedChange: EventEmitter<boolean> = new EventEmitter<boolean>(); // Output to parent
//setting for no vendor promo
    @Output() verificationsNVPSelectedChange: EventEmitter<boolean> = new EventEmitter<boolean>(); // Output to parent
    @Output() pozativeSelectedChange: EventEmitter<boolean> = new EventEmitter<boolean>(); // Output to parent
  ...
  ngOnInit(): void {
    ...
    if(this.whichOneToShow=='old_package' || this.whichOneToShow=='adit_lite_and_tech_analytics'){
  ...
  oldPacakSelection(packageSelected:any){
    if(packageSelected=='tech'|| packageSelected=='tech_withAditLite'){
      ...
      this.aditLiteSelected=false
      this.getPricingOverallForOld_pacakge()
    }else if(packageSelected=='analytic' || packageSelected=='analytic_withAditLite'){
      this.analyticSelected=true
      this.techSelected=false
      this.aditLiteSelected=false
      this.getPricingOverallForOld_pacakge()
  ...
  getPricingOverallForOld_pacakge(){
    ...
    keysWithValues.forEach((item) => {
      ...
      if( item.key=='techMonthly'||item.key=='analyticAnnual' || item.key=='techAnnual' || item.key=='analyticMonthly' || item.key=='aditLiteMontly' || item.key=='aditLiteAnnual'){
        return
      }
    ...
    if(this.multiple_location=='yes'){
      ...
      if(this.techSelected){
        ...
        this.totalMonthly.emit(this.newPackageTotalMonthly)
      }else  if(!this.analyticSelected && !this.techSelected && !this.aditLiteSelected){
  ...
  getPricingOverallForOtherPacakge(){
    ...
    keysWithValues.forEach((item) => {
      ...
      if (isNaN(numericValue)) return; // Skip invalid values
      if( item.key=='hardwareCreditAnnually'|| item.key=='hardwareCreditMonthly' || item.key=='aditLiteMontly' || item.key=="aditLiteAnnual"){
        return
      }
  ...
  packagePrice:any
   selectpackage(pacakgeName:string){
      ...
      if (pacakgeName === 'aditCore') {
        ...
        this.pozativeSelected = false;
        this.verificationsSelected = false;
        this.newPackageTotalMonthlyForNoVendorPromo = parseInt(this.pricingArray.aditCore_monthly) +parseInt(this.pricingArray.add_on_phones)+parseInt(this.pricingArray.add_on_analytic) + parseInt(this.pricingArray.add_on_verification)
        this.newPackageTotalAnnuallyForNoVendorPromo = parseInt(this.pricingArray.aditCore_annually) +parseInt(this.pricingArray.add_on_phones)+parseInt(this.pricingArray.add_on_analytic) + parseInt(this.pricingArray.add_on_verification)
